...
import { deslogarUsuario, LoginService, obterUsuario } from "../../hooks/api";
...
export const AuthProvider = ({ children }: { children: JSX.Element }) => {
  ...
  const logar = async (email: string, senha: string, tipo: string) => {
    console.log(tipo);
    if (tipo == "Aluno") {
      const loginData = await LoginService.login(email, senha, tipo);
      if (loginData.token) {
    ...
    if (tipo == "Empresa") {
      const data = await LoginService.login(email, senha, tipo);
      if (data.token) {
